class AccountUsersController < ApplicationController
  ...
  def create
    ...
    respond_to do |format|
      if @account_user.save
        format.html { redirect_to :action => :index, notice: 'Account user was successfully created.' }
        format.json { render action: 'show', status: :created, location: @account_user }
      else
  ...
  def update
    respond_to do |format|
      if @account_user.update(account_user_params)
        format.html { redirect_to :action => :index, notice: 'Account user was successfully updated.' }
  ...
  private
    ...
    def account_user_params
      params.require(:account_user).permit(:account_id, :user_id, :permission_level)
    end
end
